...
# Frequency model using LGBM
freq_lgbm = lightgbm.LGBMRegressor(objective="tweedie", tweedie_variance_power=1.5)
freq_lgbm.fit(X_train_p, y_train_p, sample_weight=w_train_p)

# Severity model using LGBM
sev_lgbm = lightgbm.LGBMRegressor(objective="tweedie", tweedie_variance_power=1.5)
sev_lgbm.fit(X_train_g, y_train_g, sample_weight=w_train_g)

# Evaluate Frequency model
freq_train_pred = freq_lgbm.predict(X_train_p)
freq_test_pred = freq_lgbm.predict(X_test_p)
...
sev_train_pred = sev_lgbm.predict(X_train_g)
sev_test_pred = sev_lgbm.predict(X_test_g)

print('LGBM severity model training loss: {}'.format(
    LgbmDist.deviance(y_train_g, sev_train_pred, sample_weight=w_train_g) / np.sum(w_train_g)
))
print('LGBM severity model test loss: {}'.format(
    LgbmDist.deviance(y_test_g, sev_test_pred, sample_weight=w_test_g) / np.sum(w_test_g)
))
...
print("Total claim amount on train set, observed = {}, predicted = {}".format(
    ...
    np.sum(df['Exposure'].values[train] * freq_train_pred * sev_lgbm.predict(X_train_p))
))

print("Total claim amount on test set, observed = {}, predicted = {}".format(
    df['ClaimAmountCut'].values[test].sum(),
    np.sum(df['Exposure'].values[test] * freq_test_pred * sev_lgbm.predict(X_test_p))
))
